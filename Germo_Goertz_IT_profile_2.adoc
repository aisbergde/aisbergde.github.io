...
// default für jekyll auf Server env-server: /assets/img
// env-local: ./assets/img
...
// folgendes kann zusammen mit dem Attribute imagesdir getestet werden. +
// If an image macro’s target is an absolute path or URL, the value of imagesdir is not added to the target path. +
// Man müsste das mit einer Option versehen und den jeweiligen Konverter abfragen +
// https://docs.asciidoctor.org/asciidoc/latest/attributes/document-attributes-ref/#intrinsic-attributes[] +
// aktueller Inhalt: `./assets/img`

// `\{backend\}` {backend}

// ist beim Export in Word: `docbook5` +
// und im Web? `html5`


// `\{basebackend\}` {basebackend}

// ist beim Export in Word: `docbook` +
// und im Web? `html`

// Das ist es zwar auch im VSC Browser, aber dann ist das eben so.
...
// Und jetzt den Inhalt des Attributs ändern in `/assets/img`
